...
class Client:
    ...
    def order_book(self, symbol,parameters={'limit_bids':1,'limit_asks':1}):
        """
        ...
        """
        #parameters={}
        # parameters['limit_bids']=1
        # parameters['limit_asks'] = 1
    ...
    def get_depth(self,symbol):
        depth=self.order_book(symbol,parameters={'limit_bids':50,'limit_asks':50})
        #print depth['bids'][0]
...
def write_log(path,content):
    path=path+'_'+time.strftime('%Y-%m-%d', time.localtime(time.time()))+'.txt'
    #print path
    with open(path,'a') as log_file:
        log_file.writelines(str(content)+'\n')

if __name__ == '__main__':
    bitfinex_rest=Client()
    last_ask_price = 0
    last_bid_price = 0
    last_ask_quant = 0
    last_bid_quant = 0
    while 1:
        try:
            ...
            depth=bitfinex_rest.get_depth('btcusd')
            if depth:
                for i in range(50):
                    ask_quant = ask_quant + float(depth['asks'][i]['amount'])
                    bid_quant = bid_quant + float(depth['bids'][i]['amount'])
                ask_price=float(depth['asks'][0]['price'])
                bid_price=float(depth['bids'][0]['price'])
                content= ask_price, ask_quant,bid_price, bid_quant
                write_log('bitfinex.txt',content)
                print content
                ...
                #
                #
                # if (last_ask_quant > last_bid_quant):
                #     if direct_ask == 1:
                #         print 'ask','right'
                #     elif direct_ask== -1:
                #         print 'ask','wrong'
                #     # else:
                #     #     print 'ask','not change'
                #
                #     if direct_bid ==1:
                #         print 'bid', 'right'
                #     elif direct_ask == -1:
                ...
                # last_ask_quant=ask_quant
                # last_bid_quant=bid_quant
                # #print 1111,last_ask_price,last_bid_price,last_ask_quant,last_bid_quant
