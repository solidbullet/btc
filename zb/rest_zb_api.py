...
class ZApi:
    ...
    def digest(self, arg_value):
        ...
        dg = h.hexdigest()
        return dg
    def get_depth(self,symbol,layer,innerlay):
        depth=(self.depth(symbol,layer))
        #print(depth)
        if depth:
            if 'asks' in depth and 'bids' in depth:
                depth['asks'] = depth['asks'][::-1]
                if innerlay>0:
                    depth['asks']=depth['asks'][innerlay:]
                    depth['bids'] = depth['bids'][innerlay:]
                return(depth)
    ...
    def get_symbol_info(self, symbol):
        ...
        if markets:
            if symbol in markets:
                print symbol
                symbol_info = {'price_precise': markets[symbol]['priceScale'],
                               'amount_precise': markets[symbol]['amountScale'],
                               'minsize': 10 ** (-1 * markets[symbol]['amountScale'])}
                return symbol_info

    def get_ticker_last(self, symbol):
        tick = float(self.ticker(symbol)['ticker']['last'])
        if tick:
            return tick
    ...
    def get_outer_layers(self, symbol, inner_layer):
         # if inner_layer<1:
         #     return False
         depth = self.get_depth(symbol,0)
         # print depth['asks']
         # print depth['bids']
         if 'bids' in depth and 'asks' in depth:
              if len(depth['bids']) > inner_layer and len(depth['asks']) > inner_layer:
                 outlay_depth = {'asks': depth['asks'][inner_layer:], 'bids': depth['bids'][inner_layer:]}
                 return outlay_depth

    def get_outer_layers__(self, symbol, inner_layer):
        # if inner_layer<1:
        #     return False
        depth = self.get_depth(symbol, 0)
        # print depth['asks']
    ...
    def balance_check_usdt(self):
        # markets = self.markets()
        # print(markets)
        # for sym in markets:
        #     print (sym,markets[sym])

        account_info=self.get_account_info()['result']['coins']

       # print(account_info)
        print('zb - usdt')
        for list in account_info:
            if float(list['available'])>0:

                #print(list['enName'],float(list['available']))
                pair=list['enName'].lower()+'_usdt'
                last_price=self.ticker(pair)
                if 'ticker' in last_price:
                    if list['enName'] not in {'ZB'}:

                        print list['enName'], round(float(list['available'])+float(list['freez']),8),round(float(last_price['ticker']['last']),8)
                        #print 'freez',list['freez'], 'ava',list['available']

    def balance_check_btc(self):
        # markets = self.markets()
        # print(markets)
        # for sym in markets:
        #     print (sym,markets[sym])
       ...
       # print(account_info)
        ...
        for list in account_info:
            if float(list['available']) > 0:
                ...
                # print(list['enName'],float(list['available']))
                pair = list['enName'].lower() + '_btc'
                last_price = self.ticker(pair)
                if 'ticker' in last_price:
                    if list['enName'] not in {'ZB'}:
                        print list['enName'], round(float(list['available']) + float(list['freez']), 8), round(
                            float(last_price['ticker']['last']), 8)
                        # print 'freez',list['freez'], 'ava',list['available']


    def balance_check_QC(self):
        # markets = self.markets()
        # print(markets)
        # for sym in markets:
        #     print (sym,markets[sym])
        ...
        for list in account_info:
            if float(list['available']) > 0:
                ...
                if 'ticker' in last_price:
                    ...
                    print  list['enName'], round(float(list['available']) + float(list['freez']), 8)
                        # print 'freez',list['freez'], 'ava',list['available']
...
if __name__ == '__main__':
    ...
    api = ZApi(zb_access_key, zb_access_secret)
   #  #print(api.order_send('btc_usdt','buy',0.0001,2000))
   #  # print(api.get_account_info())
   #  #print(api.get_depth('btc_usdt',50))
   #  # print(api.get_depth('btc_usdt',5))
   # # print(api.get_symbol_info('btc_qc'))
   # #  print(api.get_depth('btc_qc',5))
   #  print(api.cancel_allorder('btc_usdt',1))
   #
    # depth=api.get_depth('btc_qc',50)
    # print(depth)
    # print(depth['asks'],len(depth['asks']))
    # print(depth['bids'], len(depth['bids']))

    #api.balance_check_usdt()


    trades= api.trades('ADA_BTC')
    for i in trades:
        print trades